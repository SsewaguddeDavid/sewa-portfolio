...
//         {/* Left Side: Creative Contact Info (2 Columns) */}
...
//         {/* Right Side: The Inquiry Form (3 Columns) */}
//         <motion.div
...
//               </label>
//               <select className="w-full px-5 py-4 rounded-2xl bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 focus:ring-2 focus:ring-primary outline-none transition-all appearance-none">
...
//               className="w-full py-5 rounded-2xl bg-primary text-white font-bold flex items-center justify-center gap-3 hover:bg-primary-dark transition-all disabled:opacity-70"
//             >
//               {status === "success" ? (
//                 <>
...
export default function Contact() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (data.success) {
        setStatus("success");
        e.currentTarget.reset(); // Clears the form on success
      } else {
        setStatus("error");
      }
    } catch (error) {
